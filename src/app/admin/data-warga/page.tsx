...
export default function DataWargaPage() {
      ...
      const enrichedWarga = await Promise.all(
        (wargaData || []).map(async (warga) => {
          ...
          const completedReports =
            reportsData?.filter((r) => r.status === "selesai").length || 0;
          const rejectedReports =
            reportsData?.filter((r) => r.status === "ditolak").length || 0;
          const lastReport =
            reportsData && reportsData.length > 0
              ? reportsData[0].created_at
              : null;
  ...
  const calculateStats = (warga: WargaData[]) => {
    ...
    const total_reports = warga.reduce(
      (sum, w) => sum + (w.reports_count || 0),
      0
    );
    const pending_reports = warga.reduce(
      (sum, w) => sum + (w.pending_reports || 0),
      0
    );
    const completed_reports = warga.reduce(
      (sum, w) => sum + (w.completed_reports || 0),
      0
    );
    const rejected_reports = warga.reduce(
      (sum, w) => sum + (w.rejected_reports || 0),
      0
    );

    const avg_reports_per_warga =
      total > 0 ? Math.round((total_reports / total) * 10) / 10 : 0;
  ...
  const calculateStatsWithReports = (warga: WargaData[], reports: any[]) => {
    ...
    const updatedWarga = warga.map((w) => {
      ...
      const lastReport =
        userReports.length > 0 ? userReports[0].created_at : null;
  ...
  const applyFilters = () => {
    ...
    if (filterKecamatan !== "all") {
      filtered = filtered.filter(
        (warga) => warga.kecamatan === filterKecamatan
      );
    }
                    ...
                    <td className="py-3 px-4">
                      ...
                      {warga.last_report_date && (
                        <div className="text-xs text-gray-400 mt-1">
                          Laporan terakhir:{" "}
                          {new Date(warga.last_report_date).toLocaleDateString(
                            "id-ID"
                          )}
                        </div>
                      )}
                    ...
                    <td className="py-3 px-4">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`px-2 py-1 rounded text-xs font-medium ${getReportCountColor(
              ...
              <div>
                ...
                <ul className="space-y-1 text-sm text-gray-600">
                  <li className="flex items-center gap-2">
                    ...
                    <span>
                      <strong>Aktif:</strong> Memiliki laporan dalam 30 hari
                      terakhir
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    ...
                    <span>
                      <strong>Tidak Aktif:</strong> Tidak ada laporan dalam 30
                      hari terakhir
                    </span>
                  </li>
                </ul>
